...
class Problem {
    ...
    public static String StrongestExtension(String class_name, ArrayList<String> extensions) {
        ...
        return extensions.stream().map(a -> a.replaceAll("[^A-Za-z]", "")).map(a -> class_name + "." + a).sorted((a, b) -> {
            int capA = (int) a.chars().map(c -> c).filter(c -> c >= 65 && c <= 90).count();
            int capB = (int) b.chars().map(c -> c).filter(c -> c >= 65 && c <= 90).count();
            int smA = (int) a.chars().map(c -> c).filter(c -> c >= 97 && c <= 122).count();
            int smB = (int) b.chars().map(c -> c).filter(c -> c >= 97 && c <= 122).count();
