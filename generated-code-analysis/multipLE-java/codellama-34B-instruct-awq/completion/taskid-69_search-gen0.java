...
class Problem {
    ...
    public static long search(ArrayList<Long> lst) {
        ...
        int max = lst.stream().mapToInt(l -> l.intValue()).max().getAsInt();
        Map<Long, Long> counts = lst.stream().collect(Collectors.groupingBy(Function.identity(), Collectors.counting()));
        Long[] sorted = counts.entrySet().stream().map(Map.Entry::getKey).sorted(Comparator.reverseOrder()).toArray(Long[]::new);
        for (int i = 0; i < sorted.length; i++) {
            Long key = sorted[i];
            Long value = counts.get(key);
            if (key > 0 && value >= key) return key;
        }
        return -1L;
    }
