...
class Problem {
    ...
    public static ArrayList<Long> orderByPoints(ArrayList<Long> nums) {
        ...
        Map<Integer, List<Long>> sum_and_list = new HashMap<Integer, List<Long>>();
        for(long num: nums) {
            int num_sum = 0;
            for(int digit: ("" + num).split("")
            ) {
                num_sum += digit;
            }
            if(!sum_and_list.containsKey(num_sum)) {
                sum_and_list.put(num_sum, new ArrayList<Long>());
            }
            sum_and_list.get(num_sum).add(num);
        }
        List<Map.Entry<Integer, List<Long>>> sum_and_list_list = new ArrayList<Map.Entry<Integer, List<Long>>>();
        for(Map.Entry<Integer, List<Long>> entry: sum_and_list.entrySet()) {
            sum_and_list_list.add(entry);
        }
        sum_and_list_list.sort((lhs, rhs) -> lhs.getKey().compareTo(rhs.getKey()));
        List<Long> result = new ArrayList<Long>();
        for(Map.Entry<Integer, List<Long>> entry: sum_and_list_list) {
            result.addAll(entry.getValue());
        }
        return (ArrayList<Long>)(List<?>)result;
    }
