...
class Problem {
    ...
    public static ArrayList<Long> maximum(ArrayList<Long> arr, long k) {
        ...
        // create minheap with size k and push all the elements in arr into it
        PriorityQueue<Long> minHeap = new PriorityQueue<>();
        for (int i = 0; i < k; i++) {
            minHeap.offer(arr.get(i));
        }

        // for all the remaining elements in arr
        for (int i = k; i < arr.size(); i++) {
            // if the top of the heap is greater than the current element in arr
            if (minHeap.peek() < arr.get(i)) {
                // remove the top element from heap and insert the new element in arr
                minHeap.poll();
                minHeap.offer(arr.get(i));
            }
        }

        // return the sorted elements of minHeap
