...
class Problem {
    ...
    public static Pair<Optional<Long>, Optional<Long>> largestSmallestIntegers(ArrayList<Long> lst) {
        ...
        if(max==Optional.empty()) {
            return Pair.with(Optional.of(Optional.empty()), Optional.of(min.orElse(Optional.empty())));
        } else if(min==Optional.empty()) {
            return Pair.with(Optional.of(max.orElse(Optional.empty())), Optional.of(Optional.empty()));
        } else {
            return Pair.with(Optional.of(max.orElse(Optional.empty())), Optional.of(min.orElse(Optional.empty())));
        }
    }
