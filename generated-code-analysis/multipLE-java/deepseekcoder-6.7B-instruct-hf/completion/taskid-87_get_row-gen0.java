        ...
        for (int i = 0; i < lst.size(); i++) {
            for (int j = 0; j < lst.get(i).size(); j++) {
                if (lst.get(i).get(j) == x) {
                    result.add(Pair.with(i, (long)j));
                }
            }
        }
        result.sort(Comparator.comparing((Pair<Long, Long> p) -> p.getValue0()).thenComparing((Pair<Long, Long> p) -> p.getValue1(), Comparator.reverseOrder()));
        return result;
    }
