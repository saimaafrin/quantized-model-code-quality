...
class Problem {
    ...
    public static Pair<Optional<Long>, Optional<Long>> largestSmallestIntegers(ArrayList<Long> lst) {
        ...
        long largest = Long.MIN_VALUE, smallest = Long.MAX_VALUE;
        long largest_negative = Long.MIN_VALUE, smallest_positive = Long.MAX_VALUE;
        for (long i : lst) {
            if (i > largest) largest = i;
            if (i < smallest) smallest = i;
            if (i < 0 && i > largest_negative) largest_negative = i;
            if (i > 0 && i < smallest_positive) smallest_positive = i;
        }
        if (largest == Long.MIN_VALUE) largest = 0;
        if (smallest == Long.MAX_VALUE) smallest = 0;
        return Pair.with(Optional.of(largest_negative), Optional.of(smallest_positive));
    }
