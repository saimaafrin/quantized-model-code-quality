...
class Problem {
    ...
    public static ArrayList<Long> pluck(ArrayList<Long> arr) {
        ...
        long currentSmallestValue = 0l;
        long currentSmallestIndex = -1l;
        if (arr.size() == 0) {
            return new ArrayList<Long>(Arrays.asList());
        }
        for (long i = 0l; i < arr.size(); i++) {
            if (arr.get((int)i) % 2 == 0 && (currentSmallestValue == 0l || arr.get((int)i) < currentSmallestValue)) {
                currentSmallestValue = arr.get((int)i);
                currentSmallestIndex = i;
            }
        }
        if (currentSmallestValue != 0l) {
            return new ArrayList<Long>(Arrays.asList(currentSmallestValue, currentSmallestIndex));
        }
        return new ArrayList<Long>(Arrays.asList());
    }
