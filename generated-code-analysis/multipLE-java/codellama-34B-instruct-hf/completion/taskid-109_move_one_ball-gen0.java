...
class Problem {
    ...
    public static boolean moveOneBall(ArrayList<Long> arr) {
        ...
        for (int i = 0; i < arr.size(); i++) s[i] = arr.get(i).intValue();
        int n = arr.size();
        int min = Integer.MAX_VALUE;
        for (int i = 0; i < n; i++) min = Math.min(min, s[i]);
        for (int i = 0; i < n; i++) s[i] -= min;
        for (int i = 0; i < n; i++) s[i] %= n;
        int[] freq = new int[n];
        for (int i = 0; i < n; i++) freq[s[i]]++;
        int sum = 0;
        for (int i = 0; i < n; i++) sum += (freq[i] > 1) ? (freq[i] - 1) : 0;
        return (sum % 2 == 0);
    }
