...
class Problem {
    ...
    public static ArrayList<String> byLength(ArrayList<Long> arr) {
        ...
        Long[] longArray = arr.toArray(new Long[0]);

        // Filter out the strange numbers
        Long[] validArr = Stream.of(longArray)
            .filter(x -> x > 0 && x <= 9)
            .collect(Collectors.toList()).toArray(new Long[0]);

        // Convert Long to String array
        String[] strArray = new String[validArr.length];
        for (int i = 0; i < validArr.length; i++) {
            strArray[i] = convertToName(validArr[i]);
        }

        // Reverse the array
        String[] reversedStrArray = new String[strArray.length];
        for (int i = 0; i < strArray.length; i++) {
            reversedStrArray[i] = strArray[strArray.length - i - 1];
        }

        return new ArrayList<>(Arrays.asList(reversedStrArray));
    }
