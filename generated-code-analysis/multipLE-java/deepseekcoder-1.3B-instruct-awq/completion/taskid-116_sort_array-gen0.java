...
class Problem {
    ...
    public static ArrayList<Long> sortArray(ArrayList<Long> arr) {
        ...
        ArrayList<Long> oneCounts = new ArrayList<>();
        ArrayList<Long> result = new ArrayList<>();

        for (Long number : arr) {
            int count = 0;
            for (char c : Integer.toBinaryString(number.intValue()).toCharArray()) {
                if (c == '1') {
                    count++;
                }
            }
            oneCounts.add((long) count);
        }

        result.addAll(sort(oneCounts, Comparator.comparing(oneCounts::get).thenComparing(arr::get)));
