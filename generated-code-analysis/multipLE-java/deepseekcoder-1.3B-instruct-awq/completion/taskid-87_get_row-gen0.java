...
class Problem {
    ...
    public static ArrayList<Pair<Long, Long>> getRow(ArrayList<ArrayList<Long>> lst, long x) {
        ...
        return lst.stream().map(row -> row.stream().mapToLong(i -> i).filter(i -> i == x).count() > 0 ? Pair.with((long) row.indexOf(), (long) (row.lastIndexOf())) : Pair.with(0l, 0l)).sorted(Comparator.comparing((Pair<Long, Long> p) -> p.fst).thenComparing((Pair<Long, Long> p) -> p.snd).reversed()).collect(Collectors.toList());
    }
