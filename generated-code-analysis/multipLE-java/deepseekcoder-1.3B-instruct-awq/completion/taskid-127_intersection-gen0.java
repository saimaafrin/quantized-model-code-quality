...
class Problem {
    ...
    public static String intersection(Pair<Long, Long> interval1, Pair<Long, Long> interval2) {
        ...
        long start = Math.max(interval1.getA(), interval2.getA());
        long end = Math.min(interval1.getB(), interval2.getB());
        
        if (end < start) {
            return "NO";
        }
        long length = end - start;
        long gcd = getGcd(length, interval1.getA());
        return (gcd == 1 && length > 1) ? "YES" : "NO";
    }
