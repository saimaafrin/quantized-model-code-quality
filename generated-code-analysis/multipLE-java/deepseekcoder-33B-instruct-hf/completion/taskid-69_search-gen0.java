...
class Problem {
    ...
    public static long search(ArrayList<Long> lst) {
        ...
        Map<Long, Long> count = lst.stream().collect(Collectors.groupingBy(e -> e, Collectors.counting()));
        return count.entrySet().stream().filter(e -> e.getKey() <= e.getValue()).map(Map.Entry::getKey).reduce(Long::max).orElse((long)-1);
    }
