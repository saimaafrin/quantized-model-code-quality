...
class Problem {
    ...
    public static Pair<Optional<Long>, Optional<Long>> largestSmallestIntegers(ArrayList<Long> lst) {
        ...
        Optional<Long> a = negatives.isEmpty() ? Optional.empty() : Optional.of(Collections.max(negatives));
        Optional<Long> b = positives.isEmpty() ? Optional.empty() : Optional.of(Collections.min(positives));
